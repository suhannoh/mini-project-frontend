  ...
  return (
    <div>
      <Layout layoutType='post' >
        <main className='post-add-page'>
          <form onSubmit={handleSubmit}>
            <div className='post-add-left'>
              <h1>Post Setting</h1>
              <h3>제목</h3>
              <input value={title} style={{ border: title.trim() !== "" ? "" : "1px solid #EF4444" }}
              ...
              <div className='post-add-author' >
                <label>
                  <input type="radio" name='authorType'
            ...
            <div className='post-add-right'>
              <textarea className='post-textarea' placeholder='내용을 입력하세요 (최대 500자)' value={content}
                onChange={(e) => setContent(e.target.value)} maxLength={500}></textarea>
              <div className='post-submit-btn'>
                <button type='submit' disabled={!title.trim() || !category.trim() || !content.trim() || isSubmit}>
                  {state ? "수정" : "작성"}
                </button>
              </div>
            </div>
